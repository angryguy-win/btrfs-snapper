...
root_check() {
    if [[ "$(id -u)" != "0" ]]; then
        print_line error "ERROR! This script must be run under the 'root' user!\n"
        exit 0
    fi
}
...
arch_check
pacman_check
logo "Welcome to the Arch Linux Installation Script"
disikpart
filesystem
...
if ! grep -qs '/mnt' /proc/mounts; then
    print_line error "Drive is not mounted can not continue"
    print_line info "Rebooting in 3 Seconds ..." && sleep 1
    print_line info "Rebooting in 2 Seconds ..." && sleep 1
    print_line info "Rebooting in 1 Second ..." && sleep 1
    reboot now
fi
